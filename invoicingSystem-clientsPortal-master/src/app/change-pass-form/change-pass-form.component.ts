...
export class ChangePassFormComponent implements OnInit {
  ...
  onSubmit() {
    ...
    if(!this.changePassForm.valid || !this.sessionManager.ActiveSession()) {
      ...
      if (this.sessionManager.GetClientsId() === null || this.sessionManager.GetClientsMail() === undefined || this.sessionManager.GetClientsMail() === "") {
        errorMessage = "Parece que usted no está logueado, por favor: inice sesión";
      }

      this.onInvalidField(errorMessage);
    } else {
      this.sessionManager.ChangePassword(this.sessionManager.GetClientsMail(), this.changePassForm.controls["password"].value, this.changePassForm.controls["newPassword"].value)
      .subscribe(
        (serverResponse: ServerResponse) => {
          if (serverResponse.statusCode === 200) {
            this.passwordSuccessfullyChanged = true;
            MotionUI.animateIn($("#succesMessage"), 'scale-in-up');
            setTimeout(() => this.router.navigate(['/invoicePanel']), 2500);
